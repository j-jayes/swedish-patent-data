...
tibble(y = c(4, 2, 1),
       ...
       yend = c(1, 2, 4),
       xend = c(1, 1, 1),
       label = c("Sorts and processes mail in post office: \nSorts incoming or outgoing mail into mail rack pigeonholes or into mail sacks according to destination", 
                 "Linear sorting machines in which incoming letters are removed from a stream at selected points, for use in a post office", 
                 "Closest title")) %>% 
  ggplot(aes(x, y, label = label, colour = label)) +
  geom_point() +
  geom_label() +
...
Solar panels necessitate electricians to design and install them.
