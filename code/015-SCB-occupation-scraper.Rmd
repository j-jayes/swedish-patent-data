...
df_mod <- df_mod %>% 
  mutate(group_1 = str_extract(code, "[0-9]"))
...
pca_rec <- recipe(code ~ ., data = df_mod) %>% 
  update_role(code, group_1, new_role = "id") %>% 
  step_tokenize(english) %>% 
  # step_stopwords(english) %>% 
  step_tokenfilter(english, max_tokens = 50) %>% 
  step_tfidf(english) %>% 
...
pca_juiced %>% 
  ggplot(aes(PC1, PC2, colour = group_1)) +
  geom_point()
```
